...
if [ -z "$1" ]; then
    echo "Please specify the theme path."
    exit 0
fi
...
ov_left=$(grep left /root/tmp/overscan.conf|cut -d"=" -f2)
ov_top=$(grep top /root/tmp/overscan.conf|cut -d"=" -f2)
ov_right=$(grep right /root/tmp/overscan.conf|cut -d"=" -f2)
ov_bottom=$(grep bottom /root/tmp/overscan.conf|cut -d"=" -f2)
...
echo "overscan top $ov_top"
echo "overscan left $ov_left"
echo "overscan bottom $ov_bottom"
echo "overscan right $ov_right"
...
if [ "$rotate" == "\"right\"" ] ; then
    ...
    scale_img $theme/background.png $res_y $res_x 0 /tmp/theme_scaled/background.bmp > /dev/null
    ...
    scale_img $theme/background.png $res_x $res_y 0 /tmp/theme_scaled/background.bmp > /dev/null
...
#    video_x1=$(mult $snap_x1 $scale_x)
    ...
    video_x1=$( sub $(mult $snap_x1 $scale_x) $(add $ov_left $video_offset_x1) )
    # Bei der Nutzung on overscan sind Skalierungsartefakte zu erwarten. 
    # Dadurch kann lediglich eine Anaeherung der Postion der Videos 
    # berechnet werden. 
    # hier ein Versuch mit offset = 2
 
    
 
    #video_x1=$( add $(mult $snap_x1 $scale_x) $(divide $ov_left 2) )
    #video_x1=$( add $(mult $snap_x1 $scale_x) $(sub $(divide $ov_left $ratio) $( divide $ov_left 2)) )

    video_y1=$( add $(mult $snap_y1 $scale_y) $(sub $(mult $ov_top $ratio) $( divide $ov_top 2)) )
    ...
    video_x2=$( sub $(add $video_x1 $dim_x) $(sub $( divide $ov_right $ratio) $video_offset_x2 )) 
    video_y2=$( sub $(add $video_y1 $dim_y) $( divide $ov_bottom $ratio))
...
sed -i "s/background.*/background = \"background.bmp\"/" /tmp/theme_scaled/theme.conf
